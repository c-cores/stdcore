...
TEST(index_list_struct, base_constructor)
{
	index_list<int> x;
	EXPECT_EQ(x.size(), 0);
...
{
	index_list<int> x = fill<int>(20, 10);
	EXPECT_EQ(20, x.size());
	for (int i = 0; i < x.size(); i++)
		EXPECT_EQ(x[i], 10);
}

TEST(index_list_struct, sparse_range_constructor)
{
...
TEST(index_list_struct, pop)
{
	index_list<int> x = sparse_range<int>(0, 10);
	index_list<int> y = x.pop_back(5);
	EXPECT_EQ(sparse_range<int>(0, 5), x);
...
TEST(index_list_struct, push_back)
{
	index_list<int> x;
	EXPECT_EQ(0, x.size());
	for (int i = 0; i < 10; i++)
...
TEST(index_list_struct, push_front)
{
	index_list<int> x;
	EXPECT_EQ(0, x.size());
	for (int i = 0; i < 10; i++)
...
TEST(index_list_struct, append_back)
{
	index_list<int> x = fill<int>(1, 8);
	index_list<int> y = fill<int>(1, 3);
	index_list<int> z;
	EXPECT_EQ(1, x.size());
	EXPECT_EQ(1, y.size());
	...
	z = x;
	x.append_back(index_list<int>());
	EXPECT_EQ(z, x);
}

TEST(index_list_struct, append_front)
{
	index_list<int> x = fill<int>(1, 8);
	index_list<int> y = fill<int>(1, 3);
	index_list<int> z;
	...
	z = x;
	x.append_front(index_list<int>());
	EXPECT_EQ(z, x);
}

TEST(index_list_struct, replace)
{
	index_list<int> x = sparse_range<int>(0, 10);
...
TEST(index_list_struct, replace_container)
{
	index_list<int> x = sparse_range<int>(0, 10);
	EXPECT_EQ(x, sparse_range<int>(0, 10));
...
TEST(index_list_struct, swap)
{
	index_list<int> x = sparse_range<int>(0, 10);
	index_list<int> y = fill<int>(8, 5);
	EXPECT_EQ(sparse_range<int>(0, 10), x);
...
TEST(index_list_struct, release)
{
	index_list<int> x = sparse_range<int>(0, 10);
	EXPECT_EQ(10, x.size());
...
TEST(index_list_struct, compare)
{
	index_list<int> x = sparse_range<int>(0, 10);
	index_list<int> y = sparse_range<int>(10, 0, -1);
